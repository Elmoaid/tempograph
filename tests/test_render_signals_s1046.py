...
class TestBrokerWarningIntegration:
    ...
    def test_fires_for_build_graph(self):
        """build_graph in builder.py is a bidirectional hub — broker fires."""
        from tempograph import build_graph as _build_graph
        from tempograph.render.focused import _compute_broker_warning
        ...
        for s in graph.symbols.values():
            if s.name == "build_graph" and "builder.py" in s.file_path:
                sym = s
                break
        assert sym is not None, "build_graph not found in graph"

        result = _compute_broker_warning([sym], graph)
        assert "broker" in result, f"Expected broker to fire for build_graph, got: {result!r}"

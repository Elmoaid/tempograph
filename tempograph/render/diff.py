...
def _graph_a_tests_and_coverage(normalized: set[str], lines: list[str]) -> None:
    ...
    if 1 <= len(_tests_in_diff) <= 4:
        lines.append(f"Tests in diff: {', '.join(_tests_in_diff)} ✓")
...
def _graph_a_file_types_a(
    changed_files: list[str], normalized: set[str], lines: list[str]
) -> None:
    _all_files = list(normalized) + [f for f in changed_files if f not in normalized]
    ...
    # S199: Focused change — the diff touches only 1 source file.
    ...
    if _s235_schema_files:
        _sch_str = ", ".join(fp.rsplit("/", 1)[-1] for fp in _s235_schema_files[:3])
        lines.append(
...
def _graph_a_file_types_b(
    changed_files: list[str], normalized: set[str], lines: list[str]
) -> None:
    _all_files = list(normalized) + [f for f in changed_files if f not in normalized]

    # S240: Changelog/release file in diff.
    ...
    _s240_files = [fp for fp in _all_files if fp.rsplit("/", 1)[-1].lower() in _s240_changelog_names]
    if _s240_files:
        _cl_str = ", ".join(fp.rsplit("/", 1)[-1] for fp in _s240_files[:2])
        lines.append(
            f"release commit: {_cl_str} in diff"
            f" — treat as public release; changes must be backward-compatible"
        )

    # S245: Infrastructure/environment file in diff.
    _s245_infra_names = {
    ...
    _s245_files = [
        fp for fp in _all_files
        if (
            fp.rsplit("/", 1)[-1].lower() in _s245_infra_names
            or any(fp.lower().startswith(p) or f"/{p}" in fp.lower() for p in _s245_infra_paths)
        )
    ]
    if _s245_files:
        _inf_str = ", ".join(fp.rsplit("/", 1)[-1] for fp in _s245_files[:3])
    ...
    # S254: Migration file in diff (by directory path).
    ...
    if _s254_mig_files:
        _mig_str = ", ".join(fp.rsplit("/", 1)[-1] for fp in _s254_mig_files[:3])
    ...
    # S273: Documentation file in diff.
    _s273_doc_names = {
    ...
    _s273_doc_files = [
        fp for fp in _all_files
        if (
            fp.rsplit("/", 1)[-1].lower() in _s273_doc_names
            or any(fp.lower().startswith(d) or f"/{d}" in fp.lower() for d in _s273_doc_dirs)
        )
    ]
    if _s273_doc_files:
        _doc_str273 = ", ".join(fp.rsplit("/", 1)[-1] for fp in _s273_doc_files[:3])
        lines.append(
...
def _graph_a_scope_a(
    ...
    if _ep_changed:
        _ep_str = ", ".join(fp.rsplit("/", 1)[-1] for fp in _ep_changed)
...
def _graph_a_scope_b(
    graph: "Tempo", changed_files: list[str], normalized: set[str], lines: list[str]
) -> None:
    # S211: Missing co-editors — files that historically change WITH the diff files but absent.
    if graph.root:
        try:
    ...
    if len(_s215_src_files) >= 5:
        _s215_str = ", ".join(fp.rsplit("/", 1)[-1] for fp in _s215_src_files[:3])
    ...
    if len(_s261_top_dirs) >= 3:
        _dir_str261 = ", ".join(sorted(_s261_top_dirs)[:3])
        if len(_s261_top_dirs) > 3:
...
def _signals_diff_graph_a(
    ...
    _graph_a_tests_and_coverage(normalized, lines)
    _graph_a_graph_tests(graph, normalized, lines)
    _graph_a_sym_changed_syms(graph, normalized, _all_changed_syms, lines)
    _graph_a_sym_file_impact(graph, normalized, lines)
    _graph_a_file_types_a(changed_files, normalized, lines)
    _graph_a_file_types_b(changed_files, normalized, lines)
    _graph_a_scope_a(graph, normalized, lines)
    _graph_a_scope_b(graph, changed_files, normalized, lines)
